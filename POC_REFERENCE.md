# NASA Patent Matching Tool -- POC Reference

This document covers the proof-of-concept implementation for the NASA Patent Similarity Search tool. It's intended as a reference for the team: each section explains a specific problem, the approach taken, and the relevant code so anyone can pick up, reuse, or improve a piece without re-reading the full codebase.
...
3. [Patent Number Normalization](#3-patent-number-normalization)
4. [Data Layer -- BigQuery Vector Search](#4-data-layer----bigquery-vector-search)
5. [Citation Expansion Pipeline](#5-citation-expansion-pipeline)
6. [Graph Ranking -- Personalized PageRank](#6-graph-ranking----personalized-pagerank)
7. [Score Blending](#7-score-blending)
8. [LLM Integration -- Gemini Summarization](#8-llm-integration----gemini-summarization)
9. [Network Graph Visualization](#9-network-graph-visualization)
10. [Analytics Charts](#10-analytics-charts)
11. [Frontend -- Streamlit Application](#11-frontend----streamlit-application)
12. [Print / PDF Export Styles](#12-print--pdf-export-styles)
13. [Caching Strategy](#13-caching-strategy)
14. [Error Handling and Graceful Degradation](#14-error-handling-and-graceful-degradation)
...
## 4. Data Layer -- BigQuery Vector Search
...
## 6. Graph Ranking -- Personalized PageRank
...
## 8. LLM Integration -- Gemini Summarization
...
## 11. Frontend -- Streamlit Application
...
The search pipeline now starts with a normalization step before querying BigQuery. The status indicator shows intermediate feedback like "Resolving patent number..." and "Resolved `8410469` → `US-8410469-B2`" so the user understands the lookup.
...
The 1-hour TTL means the first search for a given patent takes ~6-8 seconds. Subsequent searches for the same patent within the hour return in under 1 second.
...
Cloud Build handles the Docker build. Cloud Run serves the container. No CI/CD pipeline is needed for the POC -- this single command does everything.
...
**Comparison to previous system:** The old architecture (Fall 2025) used a 104GB RAM VM running at $800-1,200/month with 2-5 minute query times. This serverless approach costs $35-40/month with 6-8 second queries.
